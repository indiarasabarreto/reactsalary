...
function discountINSSCalculator(baseINSS) {
  let discountINSS = 0;
...
function discountIRPFCalculator(baseIRPF) {
  let discountIRPF = 
    baseIRPF < 1903.98
    ? 0
    : baseIRPF < 2826.65
...
function calculateSalaryFrom(grossSalary) {
  ...
  const discountINSS = discountINSSCalculator(baseINSS);

  const baseIRPF = baseINSS - discountINSS;
  const discountIRPF = discountIRPFCalculator(baseIRPF);
